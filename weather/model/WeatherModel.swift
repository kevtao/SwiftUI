...
class WeatherModel {
    static func getTemperature(_ cityName: String, _ onComplete: @escaping (WeatherItem) -> Void) {
        guard let url = URL(string: "https://bigwhale.herokuapp.com/api/location/search/?query=" + cityName.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)!) else { return }
        URLSession.shared.dataTask(with: url) { data, _, _ in
            guard let data = data, let locations = try? JSONDecoder().decode([Location].self, from: data) else { return }
            guard locations.count > 0, let url = URL(string: "https://bigwhale.herokuapp.com/api/location/" + String(locations[0].woeid)) else { return }
            URLSession.shared.dataTask(with: url) {data, _, _ in
